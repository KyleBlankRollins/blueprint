---
title: Theme Plugin Best Practices
description: Advanced patterns and recommendations for creating high-quality theme plugins
category: guides
order: 3
---
...
**Complete list of semantic tokens:** See [Semantic Tokens Reference](/themes/plugin-authoring-guide#semantic-tokens-reference) in the Plugin Authoring Guide.
...
const cachedColors: Record<string, ColorDefinition> = {};
...
export class MyTheme extends ThemeBase {
  ...
  async register(builder: ThemeBuilderInterface): Promise<void> {
...
```typescript
// ocean-colors.ts - Defines ocean color palette
export class OceanColorsTheme extends ThemeBase {
  readonly id = 'ocean-colors';
  readonly version = '1.0.0';
...
export const oceanColorsTheme = new OceanColorsTheme();

// ocean-variants.ts - Creates theme variants
export class OceanVariantsTheme extends ThemeBase {
  readonly id = 'ocean-variants';
  readonly version = '1.0.0';
  readonly dependencies = [{ id: 'ocean-colors' }];
...
export const oceanVariantsTheme = new OceanVariantsTheme();
```
...
```typescript
export class OceanDarkTheme extends ThemeBase {
  readonly id = 'ocean-dark';
  readonly version = '1.0.0';

  // Explicit dependencies
  readonly dependencies = [{ id: 'ocean', version: '^1.0.0' }];
...
export const oceanDarkTheme = new OceanDarkTheme();
```
...
export class OceanTheme extends ThemeBase {
  ...
  validate(config: ThemeConfig) {
    return validateOceanTheme(config);
  }
}
...
export function createOceanTheme(config: OceanThemeConfig = {}): OceanTheme {
  const { primaryHue = 220, accentHue = 180, includeVariants = true } = config;

  return new (class extends ThemeBase {
    readonly id = 'ocean';
    readonly version = '1.0.0';
...
export class OceanTheme extends ThemeBase {
  ...
  readonly description = 'Calming ocean-inspired color palette';
  readonly author = 'Your Name <email@example.com>';
  readonly tags = ['blue', 'ocean', 'calm', 'professional'];
...
```typescript
export class OceanCustomTheme extends ThemeBase {
  readonly id = 'ocean-custom';
  readonly version = '1.0.0';
...
export const oceanCustomTheme = new OceanCustomTheme();
```
...
- [API Reference](/themes/api-reference) - Complete API documentation
- [Type Generation](/themes/type-generation) - TypeScript type generation
