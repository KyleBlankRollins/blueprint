...
```javascript
import '@krollins/blueprint/radio';
```
...
<ComponentPreview>
  <div style="display: flex; flex-direction: column; gap: 0.5rem;">
    ...
    <bp-radio name="error-demo" value="2" error>Another option with error</bp-radio>
  </div>
</ComponentPreview>
...
| ----------- | ------------- | ------------------------------------ |
| `bp-change` | `{ checked, value }` | Fired when the checked state changes |
